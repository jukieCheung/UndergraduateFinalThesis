...
data = np.load('/home/jukie/edfxsc/new_file.npz')
#data = np.load('/home/jukie/new_file11111.npz')
#data = np.load('/home/jukie/edfxsc/test3file.npz')
#data = np.load('/home/jukie/edfxsc/test_file122.npz')
#data = np.load('/home/jukie/edfxsc/test_file2.npz')

#data = np.load('/home/jukie/edfxsc/SC4001.npz')
orig_x = data['x']
...
x_train, x_test, y_train, y_test = \
    train_test_split(x, y, test_size=0.3,random_state=1025)
...
for epoch in range(epochs_t):
    ...
    with tqdm.tqdm(total=iter_per_epoch) as pbar:
        for X, y in train_iter:
            ...
            #print(predicted)
            optimizer.zero_grad()
            #l = criterion(torch.argmax(output,dim=1),torch.tensor(y,dtype=torch.int64).cuda())
